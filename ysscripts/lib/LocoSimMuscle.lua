...
--require('IPC_based/testmw_save') dbg_testmw=true preciseComparisonSimul={0,5} preciseComparison={0,5}

package.path=package.path..";../Samples/ysscripts/lib/?.lua" --;"..package.path
require('QPservo2')
require("LocoSynthMuscle")
...
function OnlineLocoSynthesis:setFloor(sim)
	...
	for i,floor in ipairs(floors) do
		-- ys
		registerContactPairAll(model, mLoader, floor, sim)
...
function OnlineLocoSynthesis:__init()
	...
	self.theta=vectorn()
	self.dtheta=vectorn()
	OnlineSynthesisMuscle.__init(self)
	...
	if drawRefPose == true then
		self.skin2:setVisible(true)
...
--	self.simulator:createObstacle(mesh)
	...
	-- dcjo modify initial pos
	--initialState:set(11, math.rad(45))
	...
	if useCase.speedMod then
		initialVel:setVec3(0, initialVel:toVector3(0)+useCase.speedMod)
	end
	...
	if useCase.device ~= nil then
		self.device = useCase.device(self, mOsim, timestep)
	else
		self.device = nil
	end
	...
	--dcjo surgery
	if useCase.muscleModification ~= nil then
		for i=1,#useCase.muscleModification do
			local mod_info = useCase.muscleModification[i]
			local i_mscl = mOsim:getMuscleIndexWithName(mod_info[1])
			mOsim[mod_info[2]][i_mscl] = mOsim[mod_info[2]][i_mscl] + mod_info[3]
		end
	end
	...
	self.pdservo=QPservo2(self.skel_withoutCOMjoint.dofInfo, timestep, Physics.DynamicsSimulator.EULER, self.simulator, model, self.device) 
...
-- dcjo
function OnlineLocoSynthesis:resetInitialState()
	...
	if simulStart>=0 then   
		...
		if useCase.numericalDerivDmot then
			assert(math.mod(simulFrameRate,frameRate)==0) 
				-- if error occurs, next onestepsimul function assert at this. numSimulFrames == simulStart +1
...
--[[
		if rank==nil then
			print("refTime")
			print(self.outputGlobal.refTime(currFrame))
		end
]]
		for currSimulFrame=simulStart,simulEnd-1 do	 
			...
			local function computeTorque()
				do -- position controller
					...
					if useQPsolver then
						...
						-- actually qpservo
						-- dcjo debug. PANIC point of simulator
						if idservo:_calcDesiredAcceleration(self)==false then
							self.errorOccurred=true
							error_feedback_method=EFM.NONE -- fall down.
							print("idservo:_calcDesiredAcceleration(self)==false. in LocoSimMuscle")
							return
						end
						...
						--##dos
						if g_debugOneStep then --##dos
						g_debugOneStep:pushBack({"addb",footPosL:copy(), lfoot:copy(), k_p, footPosL2:copy(), footPosL:copy(), lfootvel:copy(), k_d,impL}) --##dos
						end
			...
			if self.impulse>0 then
				...
				--local chest=self.skel_withoutCOMjoint:VRMLbone(self.skel_withoutCOMjoint:getBoneByVoca(MotionLoader.CHEST):treeIndex())
				local chest=self.skel_withoutCOMjoint:VRMLbone(10)
				--local chest=self.skel_withoutCOMjoint:VRMLbone(self.skel_withoutCOMjoint:getBoneByName('knee_l'):treeIndex())
				--print(chest)
				--dbg.console()
			...
			if useCase.keyframes and useCase.keyframes.feedforwardTorque then
				...
				--##dos
			if g_debugOneStep then --##dos
			local qd=self.simulator:queryContactDepths() g_debugOneStep:pushBack({"queryContactDepth",qd:copy(), self.contactState.contactDepth:copy()}) g_debugOneStep:pushBack({"bfullstate",deepCopyTable(self:getStates())}) --##dos
			end

			local computedTorque=computeTorque() -- QPsolve & addBoneObjective2
			for i=1, model.simulationSubTimesteps do
				if self.pdservo.stepSimul then
					...
					if self.simulator:stepSimulation()==false then
						self.errorOccurred=true
						error_feedback_method=EFM.NONE
						print("self.simulator:stepSimulation()==false. in LcoSimMuscle")
						return
					end
				end
			end
			...
			self.contactState.contactDepth:radd(qd)
							...
							--##dos
			if g_debugOneStep then --##dos
			g_debugOneStep:pushBack({"queryContactDepth",qd:copy(), self.contactState.contactDepth:copy()}) --##dos
			g_debugOneStep:pushBack({"fullstate",deepCopyTable(self:getStates())}) --##dos
			end
		...
		--ys
		--if cleanVisualize~=true then
		if g_drawAdditionalSkins==true then
			self.targetSkin:setPoseDOF(self.pdservo.theta_d)  
	...
	if mrdMotion then
		...
		do 
			...
			elseif select(1, string.find(segName,'LR')) then
				state:set(currFrame,0, 0.5)
			elseif select(1, string.find(segName,'RL')) then
				state:set(currFrame,0, 0.5)
			elseif select(1, string.find(segName,'L')) then
				state:set(currFrame,0, 0)
			elseif select(1, string.find(segName,'R')) then
				state:set(currFrame,0, 1)
			else
		...
		--activation
		if signals.activations==nil then
			signals.activations=CT.zeros(currFrame+1, mOsim:getNumMuscles())
		else
			signals.activations:resize(currFrame+1, mOsim:getNumMuscles())
		end
		signals.activations:row(currFrame):assign(tbl2vecn(mOsim:getActivations()))

		--musculotendon length
		if signals.l_mt==nil then
			signals.l_mt=CT.zeros(currFrame+1, mOsim:getNumMuscles())
		else
			signals.l_mt:resize(currFrame+1, mOsim:getNumMuscles())
		end
		signals.l_mt:row(currFrame):assign(tbl2vecn(mOsim:getTendonMuscleLengths()))
...
function OnlineLocoSynthesis:prepareNextStep() 
	...
	if self.errorOccurred or error_feedback_method==EFM.NONE then 
		...
		error_feedback_method=EFM.NONE
		self.errorOccurred=true
		print("OnlineLocoSynthesis:prepareNextStep() errorOccurred. in LocoSimMuscle")
...
function OnlineLocoSynthesis:oneStep()
	...
	OnlineSynthesisMuscle.oneStep(self)
