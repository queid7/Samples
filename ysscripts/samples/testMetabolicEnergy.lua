...
--[[
	...
	local relatedMuscles = mOsim:getMuscleRelatedJoints(joints)
	for i=1, #relatedMuscles do 
		printtblh(reltaedMuscles[i])
	end
]]--
...
--[[
	...
	local distanceData_angle = {}

	local maxRefTime = 5.0

	for angle=7, 85, 1 do
		print("test angle: "..angle)
	
		useCases.accumulate(useCases.g2592_surgery, {['keyframe,1,ankleLmod,g2592_surgery,R1,x']=math.rad(angle),['keyframe,1,ankleRmod,g2592_surgery,L,x']=math.rad(angle)})
		...
		while(mSynthesis.outputGlobal:getRefTime() < maxRefTime) do
			...
			local e = mOsim:getMetabolicEnergyRate()	
			totalEnergy = totalEnergy + e

			for mscl_i = 1, #mOsim.msclname do 
				local me = mOsim:getPartialMetabolicEnergyRateOfMuscles({mscl_i})
				local mevn = tbl2vecn(me)
				if msclEnergy[mscl_i] == nil then
					msclEnergy[mscl_i] = mevn 
				else 
					msclEnergy[mscl_i] = msclEnergy[mscl_i] + mevn
				end
			end
		...
		for mscl_i = 1, #mOsim.msclname do 
			msclEnergy[mscl_i] = msclEnergy[mscl_i] / mSynthesis.numFrames
			msclEnergyData_mscl[mscl_i][angle] = msclEnergy[mscl_i]
		end

		msclEnergyData_angle[angle] = msclEnergy



		local output = angle.."\t"..(totalEnergy/mSynthesis.numFrames)
		util.outputToFile("printMetabolicEnergy.dat", output)

		-- distance
		local gp = mOsim.bfk:globalFrame(mOsim.mLoader:getBoneByName('ground_pelvis')):toGlobalPos(vector3(0,0,0))
		gp:setY(0)
		distanceData_angle[angle] = gp:length()		
	...
	util.saveTable(msclEnergyData_angle, "msclEnergyData_angle.tbl")
	util.saveTable(msclEnergyData_mscl, "msclEnergyData_mscl.tbl")

	util.saveTable(distanceData_angle, "distanceData_angle.tbl")
