...
--	this:create('Button', 'attach camera', 'attach camera')
	...
	MotionUtil.exportBVH(mot, "convertedBVH1019")
	...
	do
		...
		local mot = Motion(mMotionDOF)

		MotionUtil.exportBVH(mot, "convertedBVH1020_"..useCase.modelName)
	...
	local ml = {'med_gas_r','lat_gas_r', 'soleus_r', 'tib_post_r', 'flex_dig_r', 'flex_hal_r', 'tib_ant_r', 'per_brev_r', 'per_long_r', 'per_tert_r', 'ext_dig_r', 'ext_hal_r'}
	...
	local outputAllTendons_title = "frame"
	for i=1, #ml do
		outputAllTendons_title = outputAllTendons_title.."\t"..ml[i]
	end

	local outputAllTendons_tf = {}
...
--		print(posedof)
--		print(posedof:get(11))
		posedof:set(11, math.rad(ifr))
--		print(posedof:get(11))
		mOsim.bfk:setPoseDOF(posedof)
		mOsim:updatePathPointPosz()
		mOsim:updateTendonForces()
	
		outputAllTendons_tf = ""..ifr


		for i=1,#ml do
			local tf = mOsim:getTendonForce(mOsim:getMuscleIndexWithName(ml[i]))
			local tls = mOsim:getTendonMuscleLengths()
			print(tf)

			local output = ""
			output = output..ifr.."\t"..tf.."\t"..tls[mOsim:getMuscleIndexWithName(ml[i])]
			util.outputToFile("testTendon_"..ml[i]..".dat", output)
		
			outputAllTendons_tf = outputAllTendons_tf.."\t"..tf

		end

		util.outputToFile("testTendon_all_tf.dat", outputAllTendons_tf)
