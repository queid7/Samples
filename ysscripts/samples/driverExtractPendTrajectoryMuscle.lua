...
package.path=package.path..";../Samples/ysscripts/libs/?.lua" --;"..package.path
		   ...
		   if rank~=nil then
			   if mSynthesis.server then
				   if outputFileName==nil then
					   mSynthesis.server:save_table("optimizeresult.lua")
				   else
					   mSynthesis.server:save_table(outputFileName)
				   end
			   end
		   else
			   if outputFileName==nil then
				   mSynthesis:save_table("optimizeresult.lua")
			   else
				   mSynthesis:save_table(outputFileName)
			   end
		   end
		   this("exit",0)
	   end
