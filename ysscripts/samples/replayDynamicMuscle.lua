...
function loadReplay()
	...
	mOsim.records = util.loadTable('result/'..dataName..'/'..dataName..'.replay')
	g_simulated_ifr = #mOsim.records

	g_rec_gp = {}
	local gp_idx = mOsim.mLoader:getBoneByName('ground_pelvis')
	for i=1, g_simulated_ifr do
		mOsim.bfk:setPoseDOF(mOsim.records[i]["bfkPoseDOF"])
		g_rec_gp[i] = mOsim.bfk:globalFrame(gp_idx):toGlobalPos(vector3(0,0,0))
	end
...
function EVR:onFrameChanged(win, iframe)
	...
	if iframe > g_simulated_ifr then
		...
		if boolean_options.attachCamera then
			...
			if g_rec_gp[iframe]~= nil then
				local curPos = g_rec_gp[iframe] * 100
				curPos.y = 0.
				if g_rec_gp[g_prev_ifr] ~= nil then
					local prePos = g_rec_gp[g_prev_ifr] * 100
					prePos.y = 0.
					RE.viewpoint().vpos:assign(RE.viewpoint().vpos + curPos - prePos)
					RE.viewpoint().vat:assign(RE.viewpoint().vat + curPos - prePos)
				else
					RE.viewpoint().vpos:assign(mCameraInfo.vpos+curPos)
					RE.viewpoint().vat:assign(mCameraInfo.vat+curPos)
				end
