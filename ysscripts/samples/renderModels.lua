...
function ctor()
	...
	do -- set viewpoint
		...
		local vpos=vector3(2.5*100,1.5*100,-1*100)
		local vat=vector3(0,.85*100,0)
	...
	model = model_files.g2562_gait
	local initialHeight=.04

	--model = model_files.g2592_gait
	--local initialHeight=.04

	--model = model_files.full_soldier
	--local initialHeight=.07
	...
	-- fix except root joint
	mOsim.mMotionDOFcontainer.mot:matView():sub(0,0,4,0):setAllValue(0)
