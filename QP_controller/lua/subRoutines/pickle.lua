...
function Pickle:pickle_(root, max_len)
  ...
  while table.getn(self._refToTable) > savecount do
    ...
    for i, v in pairs(t) do
        s = string.format("%s[%s]=%s, ", s, self:value_(i,max_len), self:value_(v, max_len))
    end
    s = s.."}, "
...
function Pickle:value_(v,max_len)
  local vtype = type(v)
  if     vtype == "string" then return string.format("%q", v)
  elseif vtype == "number" then return v
  elseif vtype == "boolean" then return tostring(v)
  elseif vtype == "table" then return "{"..self:ref_(v,max_len).."}"
  else --error("pickle a "..type(v).." is not supported")
     return vtype
  end  
end

function Pickle:ref_(t,max_len)
