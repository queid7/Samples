...
Optimizer.methods={
	...
	CMAes_ys={nrestart=1, stddev=0.25, maxIteration=1000000000, useMPIwhenPossible=true},
	CMAes_Adapt={nrestart=1, stddev=0.25, maxIteration=1000000000, useMPIwhenPossible=true},
...
--ys, dcjo
-- unnormalize 'sol' value and return string, and set opt_dimension[] values
function Func:getSolutionTableStr(sol)
	self:unnormalize(self.opos, sol)
	for i=1, self.opt.N_opt_dimension do
		local optvar=self.opt.opt_dimension[i]
		optvar.curval=get1(self.opos, i)
	end
	...
	return "accumulate({"..outputString.."})"
end

function Func:getSolutionTableStrFromOpos(opos)

	local outputString = ""
	for i=1, self.opt.N_opt_dimension do
		local optvar = self.opt.opt_dimension[i]
		outputString = outputString.."['"..optvar.title.."']".."= "..opos(i-1)..", "
	end
	return "accumulate({"..outputString.."})"
end
...
function Optimizer:optimize()
	...
	--ys
	elseif self.method==Optimizer.methods.CMAes_ys then
		print('!!!!! CMAes_ys')
		
		-- dcjo report terms for debug
		local energyTerms = {}
		for i = 0, 15 do
			util.saveTable(energyTerms, "MSE/energyTerms_"..i..".tbl")
		end
		...
		while opt:testForTermination()=="" do
			...
			-- log
			util.outputToFile(Optimizer.outFile, 'iter '..iter)

			--  generate population & job
			opt:samplePopulation()
...
--			local output=""
			for i=1, popId:size() do
				opt:setVal(popId[i], eval[i]/func.oeval)
--				output=output..popId[i]..": "..eval[i]..", "
--				util.outputToFile(Optimizer.outFile, 'job index : '..popId[i]..'	, value :	'..eval[i])
--				util.outputToFile(Optimizer.outFile, func:getSolutionTableStrFromOpos(jobs[i])) 
			end
--			util.outputToFile(Optimizer.outFile, output)
			...
			-- log
			local bestfvrecent = opt:getBestFvRecent()
			local bestfvever = opt:getBestFvEver()

			local bestsolrecent=vectorn()
			bestsolrecent:resize(self.N_opt_dimension)
			opt:getBestSolRecent(bestsolrecent)

			local bestsolever=vectorn()
			bestsolever:resize(self.N_opt_dimension)
			opt:getBestSolEver(bestsolever)

			--util.outputToFile(Optimizer.outFile, '--cma bestfvrecent '..bestfvrecent)
			--util.outputToFile(Optimizer.outFile, '--cma bestsolrecent '..bestsolrecent:__tostring())
			util.outputToFile(Optimizer.outFile, '--real bestfvrecent '..(bestfvrecent*func.oeval))
			--util.outputToFile(Optimizer.outFile, '--real bestfvrecent_compare '..math.min(unpack(eval)))
			--util.outputToFile(Optimizer.outFile, '--real bestsolrecent')
			--util.outputToFile(Optimizer.outFile, func:getSolutionTableStr(bestsolrecent))
			--util.outputToFile(Optimizer.outFile, '')

			--util.outputToFile(Optimizer.outFile, '--cma bestfvever '..bestfvever)
			--util.outputToFile(Optimizer.outFile, '--cma bestsolever '..bestsolever:__tostring())
			--util.outputToFile(Optimizer.outFile, '--real bestfvever '..(bestfvever*func.oeval))
			--util.outputToFile(Optimizer.outFile, '--real bestsolever')
			--util.outputToFile(Optimizer.outFile, '--iter '..iter)
			
			util.outputToFile(Optimizer.outFile, '--iter:'..iter..' bestfvever:'..(bestfvever*func.oeval))
			util.outputToFile(Optimizer.outFile, func:getSolutionTableStr(bestsolever))
			util.outputToFile(Optimizer.outFile, '')

			iter=iter+1	 
		end
		util.outputToFile(Optimizer.outFile, 'END OPTIMIZATION')
		util.outputToFile(Optimizer.outFile, opt:testForTermination())

	--ys
	elseif self.method==Optimizer.methods.CMAes_Adapt then
		print('!!!!! CMAes_Adapt')

		mOsim:writeMuscleProperty('muscles.dat')
		--print('initialize muscles.dat')
		...
		while opt:testForTermination()=="" do
			...
			local output=""
			local minEval = 1000000000000000
			local minPopId
			for i=1, popId:size() do
				--print('popId', popId[i], opt:getPopulation(popId[i]))
				--opt:setVal(popId[i], eval[i]/func.oeval)
				output=output..popId[i]..": "..eval[i]..", "

				if eval[i] < minEval then
					minEval = eval[i]
					minPopId = popId[i]
				end
			end
			...
			-- log 1
			util.outputToFile(Optimizer.outFile, 'f_m_o '..table.tostring(mOsim.f_m_o))

			-----------------------------------
			-- get min sol total_a
			local minJobId = minPopId+1
			local total_a = util.loadTable('total_a_jobId'..minJobId..'.dat')
			--print('load', 'total_a_jobId'..minJobId..'.dat')
			--printtblh(total_a)
			
			-- strengthen muscle
			mOsim:strengthenMuscles('best_activation', total_a)
			mOsim:writeMuscleProperty('muscles.dat')
			mOsim:writeMuscleProperty('muscles_iter'..iter..'.dat')
			--print('write muscles.dat')
			--printtblh(mOsim.f_m_o)

			util.outputToFile(Optimizer.outFile, 'STRENGTHEN')
			-----------------------------------
			...
			if bestfvrecent==bestfvever then
				bestiter = iter
			end
			...
			mOsim:readMuscleProperty('muscles_iter'..bestiter..'.dat')
			...
			--util.outputToFile(Optimizer.outFile, '--real bestsolever')
			util.outputToFile(Optimizer.outFile, func:getSolutionTableStr(bestsolever))
			util.outputToFile(Optimizer.outFile, 'local f_m_o = '..table.tostring(mOsim.f_m_o))
			util.outputToFile(Optimizer.outFile, '')
